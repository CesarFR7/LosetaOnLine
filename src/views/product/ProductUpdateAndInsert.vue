      ...
      <div class="col-9">
        <form v-on:submit.prevent="handleSubmit">
          ...
          <div class="mt-3">
            ...
            <input type="text" v-model="productObj.tags" class="form-control"
          ...
          <div class="pt-3">
            <button class="btn btn-success m-2 w-25" :disabled="loading">
              <span v-if="loading" class="spinner-border spinner-border-sm me-2"></span>{{ productIdForUpdate ? 'Update' : 'Create' }}
            </button>
            <router-link :to="{ name: APP_ROUTE_NAMES.PRODUCT_LIST }" class="btn btn-secondary m-2 w-25"> Cancel
            </router-link>
          </div>
        </form>
      </div>
      <div class="col-3">
        <img :src="`https://placehold.co/600x400`" class="img-fluid w-100 m-3 p-3 rounded" alt="Product
        preview" />
      </div>
    </div>
  </div>
    ...
    if (!errorList.length) {
      ...
      try {
        if (productIdForUpdate) {
          ...
          showSuccess('Product updated suceessfully');
        } else {
          //Create
